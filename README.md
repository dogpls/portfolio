# Online Portfolio

This portfolio is a single-page application built to present my work.

### Features

    Responsive, mobile-first design
    Project showcase with live demos and source code links

### Tech Stack

    Front-end: HTML + CSS + JavaScript


### Contributing
This is a personal portfolio project and is not open for contributions. However, feedback and suggestions are welcome via issues.

### License
MIT

### Contact

    Email: anthony@anthonygomez.net
    GitHub: https://github.com/dogpls
    Portfolio: anthonygomez.net
